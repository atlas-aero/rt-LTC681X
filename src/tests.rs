...
fn test_start_conv_cells_acc_modes() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, get_cs_no_polling(4));
...
fn test_start_conv_cells_cell_groups() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, get_cs_no_polling(7));
    monitor.start_conv_cells(ADCMode::Normal, CellSelection::All, false).unwrap();
...
fn test_start_conv_permit_charging() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, get_cs_no_polling(2));
    monitor.start_conv_cells(ADCMode::Normal, CellSelection::All, false).unwrap();
...
fn test_start_conv_cells_sdo_polling() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, cs).enable_sdo_polling();
    monitor.start_conv_cells(ADCMode::Normal, CellSelection::All, false).unwrap();
}
...
fn test_start_conv_cs_error() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, cs);
...
fn test_read_cell_voltages_register_a() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 1> = LTC681X::ltc6813(bus, get_cs_no_polling(1));
...
fn test_read_cell_voltages_multiple_devices() {
    ...
    let mut monitor: LTC681X<_, _, _, _, _, _, _, 3> = LTC681X::ltc6813(bus, get_cs_no_polling(1));
